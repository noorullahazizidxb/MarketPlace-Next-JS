...
import { useSidebar } from "../../atoms/shadcn/sidebar";
import {
  sidebarVariants,
...
// direction is now read from useSidebarConfig() context
export function LayoutTab() {
  ...
  const handleSidebarSideSelect = (side: "left" | "right") => {
    // In RTL mode we store the inverse so the effective rendered position matches
    // what the user clicked (the flip in admin-base-layout is applied on top).
    const rawSide: "left" | "right" =
      direction === "rtl" ? (side === "left" ? "right" : "left") : side;
    updateSidebarConfig({ side: rawSide });
  };
          ...
          {sidebarVariants.map((variant) => (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div
                  className={`flex min-h-[var(--ctrl-h)] rounded border ${variant.value === "inset" ? "bg-muted" : "bg-background"}`}
                >
                  {/* Sidebar representation - smaller and more proportional */}
                  <div
                    className={`w-3 flex-shrink-0 bg-muted flex flex-col gap-0.5 p-1 ${variant.value === "floating"
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          {sidebarSideOptions.map((side) => (
            <div
              key={side.value}
              className={`relative p-4 border rounded-md cursor-pointer transition-colors ${effectiveSide === side.value
                  ? "border-primary bg-primary/10"
                  : "border-border hover:border-border/60"
                }`}
              onClick={() =>
