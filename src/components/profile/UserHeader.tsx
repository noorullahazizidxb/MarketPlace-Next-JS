...
export default function UserHeader({ user }: { user: PublicUser }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-border/[0.08] shadow-[0_8px_40px_-12px_rgba(0,0,0,0.25)]">
      {/* Background layers */}
      ...
      <motion.div
        aria-hidden
        className="absolute -z-10 right-[-8%] top-[-15%] h-80 w-80 rounded-full bg-[color-mix(in oklab, var(--primary) 12%, transparent)] blur-3xl"
      ...
      <motion.div
        aria-hidden
        className="absolute -z-10 left-[-6%] bottom-[-10%] h-64 w-64 rounded-full bg-[color-mix(in oklab, var(--accent) 10%, transparent)] blur-3xl"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <div className="relative p-6 sm:p-8 md:p-10">
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 items-start">
          ...
          >
            <div className="size-24 sm:size-32 rounded-[1.5rem] overflow-hidden border border-border bg-[color-mix(in oklab, var(--muted) 20%, transparent)] shadow-[0_16px_48px_-12px_rgba(0,0,0,0.3)]">
              {user.photo ? (
                <Image
                ...
                <div className="flex items-center -space-x-2">
                  {(user.followers || []).slice(0, 5).map((f: Follower) => (
                    <Tooltip key={f.id} content={f.fullName || "Follower"} side="top">
                      <div
                        className="size-8 rounded-full overflow-hidden border-2 border-[var(--card)] bg-[color-mix(in oklab, var(--muted) 30%, transparent)] shadow-sm"
                      >
                        {f.photo ? (
                          <Image
                            src={asset(f.photo)}
