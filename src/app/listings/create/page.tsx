...
export default function CreateListingPage() {
  ...
  const renderStep3 = () => (
    <div className="space-y-[var(--space-gap)]">
      ...
      <div
        className={cn(
          "p-[var(--space-card)] rounded-2xl border-2 border-dashed bg-[var(--card)]/40 backdrop-blur-sm transition-colors",
        ...
        {existingImages.length > 0 && (
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            {existingImages.map((img: ExistingImage, i: number) => (
              <div
                key={img.url + i}
                className="relative h-28 rounded-xl overflow-hidden border border-[var(--border)]/40 bg-[var(--card)] shadow-sm"
              >
                <Image
                  src={asset(img.url)}
                  alt={`Image ${i + 1}`}
  ...
  const renderStep4 = () => (
    <form
      ...
      className="grid grid-cols-1 md:grid-cols-2 gap-[var(--space-gap)]"
    >
  ...
  return (
    <div className="min-h-screen justify-center app-shell-page" data-app-page="listings-create">
      <div className="w-full max-w-3xl">
        <Card className="relative overflow-hidden p-[var(--space-card)] rounded-[2rem] border border-[var(--border)]/40 bg-[var(--card)]/80 backdrop-blur-2xl shadow-token-lg">
          {/* shimmer top line */}
